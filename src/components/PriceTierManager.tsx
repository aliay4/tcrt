...
import { validatePriceTiers, checkTierOverlap, validateSingleTier } from '@/utils/validators';
...
export default function PriceTierManager({ 
  ...
  const handleAddTier = async (e: React.FormEvent) => {
    ...
    const tierData = {
      ...
      price: newTier.price,
      discount_percentage: newTier.discount_percentage ? parseFloat(newTier.discount_percentage) : undefined
    };
  ...
  const handleUpdateTier = async (tierId: number, updatedData: Partial<PriceTier>) => {
    try {
      setSaving(true);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2">Fiyat kademeleri yükleniyor...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Fiyat Kademeleri</h3>
          <p className="text-sm text-gray-600">Ürün için adet bazlı fiyat kademeleri</p>
        </div>
        <button
      ...
      {showAddForm && (
        <div className="bg-gray-50 rounded-lg p-6">
          <h4 className="text-md font-semibold text-gray-900 mb-4">Yeni Fiyat Kademesi</h4>
      ...
      {tiers.length === 0 ? (
        <div className="text-center py-8 bg-gray-50 rounded-lg">
          ...
          <h4 className="text-lg font-medium text-gray-900 mb-2">Henüz fiyat kademesi yok</h4>
          <p className="text-gray-600 mb-4">Ürün için adet bazlı fiyat kademeleri ekleyebilirsiniz</p>
          <button
            onClick={() => setShowAddForm(true)}
            className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
          >
          ...
          <div className="text-sm text-blue-800">
            {tiers
              ...
              .map((tier, index) => {
                ...
                return (
                  <span key={tier.id}>
                    {formatTierRange(tier)}: ₺{tier.price.toFixed(2)} (%{discountPercentage.toFixed(1)} indirim)
